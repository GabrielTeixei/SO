...
int main (int argc, char *argv[])
{
    ...
    sh->fSt.tableClients = 0;                           //number of clients at table
    sh->fSt.tableFinishEat = 0;				//number of clients that finished eating

    sh->fSt.foodRequest = 0;				//flag of food request from client to waiter
    sh->fSt.foodOrder = 0;				//flag of food order from waiter to chef
    sh->fSt.foodReady = 0;				//flag of food ready from chef to waiter
    sh->fSt.paymentRequest = 0;				//flag of payment request from client to waiter

    sh->fSt.tableLast = -1;				//id of last client to arrive 
